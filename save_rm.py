...
class RemarkableDrawer:
    ...
    def capture_input(self):
        ...
        try:
            while True:
                ...
                if not self.is_touching and time.time() - self.last_pen_lift_time > 1:
                    self.save_image()
                    self.clear()
    ...
    def _handle_pressure(self, pressure):
        if pressure > self.pressure_threshold and not self.is_touching:
            ...
            log.debug(f"Pen touched the surface (Pressure: {pressure}, Coordinates: ({self.raw_x}, {self.raw_y}))")
        elif pressure <= self.pressure_threshold and self.is_touching:
            self.is_touching = False
            if len(self.current_stroke) > 1:
                self.strokes.append(self.current_stroke)
                log.debug(f"Stroke completed with {len(self.current_stroke)} points")
            self.current_stroke = []
            self.last_pen_lift_time = time.time()
            log.debug(f"Pen lifted from surface (Pressure: {pressure}, Coordinates: ({self.raw_x}, {self.raw_y}))")
...
def main():
    parser = argparse.ArgumentParser(description="Capture and save reMarkable Paper Pro drawings")
    ...
    try:
        ...
        print("Drawings will be saved when you lift the pen for more than 1 second.")
